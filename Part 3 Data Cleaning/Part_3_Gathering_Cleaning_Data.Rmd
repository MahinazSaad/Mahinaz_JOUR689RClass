...
SF %>% 
  ...
  filter(crime_cleaned != "homeless_complaint") |>
  filter(n > 30) %>% 
...
```{r}
#format to hoursSF$date_time <- mdy_hm(SF$call_date_time)
...
SF |>
  ...
  count(disposition1, disposition, sort = TRUE)
...
serious <- c("Arrest", "Cited", "Criminal", "Medical")

SF |>
  filter(disposition1 %in% serious) |>
  count(disposition1, sort = TRUE)
...
SF |>
  ...
  geom_bar(stat = "identity", show.legend = FALSE) +
  theme(axis.text.x = element_text(angle = 90)) +
  labs(title = "Homeless Calls by Month, San Francisco",
...
SF |>
  ...
  labs(title = "Homeless Calls by Day of the Week, San Francisco",
       subtitle = "SF PD Service Call Data, 3/2016-11/2019",
       caption = "Graphic by Wells",
       y = "Number of Calls",
       x = "Day of the Week")
...
clean_crime <- function(x) {
  case_when(
    str_detect(x, "homeless_complaint") ~ "homeless_complaint",
    str_detect(x, "aggress")            ~ "aggressive",
    str_detect(x, "drug")               ~ "drugs",
    str_detect(x, "dog")                ~ "dog",
    str_detect(x, "interview")          ~ "interview",
    str_detect(x, "intox")              ~ "intoxicated",
    str_detect(x, "music")              ~ "music",
    str_detect(x, "panhandling")        ~ "panhandling",
    str_detect(x, "poss")               ~ "possession",
    str_detect(x, "sit|lying")          ~ "sit_lying",
    str_detect(x, "sleep")              ~ "sleep",
    str_detect(x, "tent")               ~ "tent",
    str_detect(x, "trespass")           ~ "trespasser",
    TRUE ~ ""
  )
}
SF <- SF |>
  mutate(crime_cleaned3 = clean_crime(crime3),
         crime_cleaned4 = clean_crime(crime4))

# Step 3: join the three cleaned columns into one combined dataframe
SF_crimes <- SF |>
  select(crime_id, crime_cleaned, crime_cleaned3, crime_cleaned4) |>
  pivot_longer(-crime_id, names_to = "source_column", values_to = "crime") |>
  filter(!is.na(crime), crime != "") |>
  distinct(crime_id, crime)
SF_crimes |>
